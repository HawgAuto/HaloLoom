...
def test_component_manifest_has_complete_source_provenance() -> None:
    ...
    assert (
        ...
        == "9cc9884911db814c8d72d7e1219fe580f94f98849674f3b468f16caf3a8e09de"
    )
    for name in (
        "hyperloom",
        "magpie",
...
def test_release_inventory_is_only_four_images_and_two_assets() -> None:
    data = json.loads(_text("manifests/components.json"))
...
def test_turnkey_ecosystem_layer_is_exactly_pinned() -> None:
    dockerfile = _text("docker/ecosystem/Dockerfile")
    exact_refs = {
        "HYPERLOOM_REF": "a94191bfe0b548a44113e0d28c432d1bff0e3f9f",
        "MAGPIE_REF": "f25176b855aad86e38ac06442f2a5d2b8e4da21f",
...
def test_image_builder_uses_golden_compaction_and_full_release_layers() -> None:
    builder = _text("scripts/build_images.sh")
    for dockerfile in (
        "docker/full-vllm/Dockerfile",
        "docker/full-sglang/Dockerfile",
        "docker/full-quark-base/Dockerfile",
        "docker/full-vllm-release/Dockerfile",
        "docker/full-sglang-release/Dockerfile",
        "docker/ecosystem/Dockerfile",
        "docker/quark/Dockerfile",
        "docker/aiter-tools/Dockerfile",
    ):
        assert dockerfile in builder
    assert "docker/vllm-core/Dockerfile" not in builder
    assert "docker/sglang-core/Dockerfile" not in builder
    assert "LOWBIT_WHEEL_SHA256" in builder
    assert "VLLM_WHEEL_SHA256" in builder
    assert "BASE_IMAGE=\"$vllm_runtime\"" in builder
    assert "BASE_IMAGE=\"$sglang_runtime\"" in builder
